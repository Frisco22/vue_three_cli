...
export default {
  ...
  methods: {
    ...
    initThree () {
      ...
      that.renderer.setClearColor(0xeeeeee, 1.0);
    },
    // 初始化灯光
    initLight () {
      const that = this
    ...
    initVein () {
      const that = this
      that.geometry = new that.THREE.PlaneGeometry(500, 300, 1, 1);
      that.geometry.vertices[0].uv = new that.THREE.Vector2(0, 0);
      that.geometry.vertices[1].uv = new that.THREE.Vector2(2, 0);
      that.geometry.vertices[2].uv = new that.THREE.Vector2(2, 2);
      that.geometry.vertices[3].uv = new that.THREE.Vector2(0, 2);
      // 纹理坐标怎么弄
      // let texture = that.THREE.ImageUtils.loadTexture(`${that.publicPath}img/vein.jpg`, null, () => {
      // });
      ...
      // that.scene.add(mesh);
      //
      // that.geometry = new that.THREE.CubeGeometry(150, 150, 150);
      // let texture = new that.THREE.Texture(canvas);
      // var material = new that.THREE.MeshBasicMaterial({ map: that.texture });
      // texture.needsUpdate = true;
      // that.mesh = new that.THREE.Mesh(that.geometry, material);
      // that.scene.add(that.mesh);
